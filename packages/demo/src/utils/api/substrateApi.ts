...
export class substrateApi {
  ...
  public async signMessage ( address: string, provider: SubstrateProvider,  signer ?: Signer, genesisHash ?: string, messageString?: string ) {
    if(signer && signer.signRaw) {
      const signPromise = signer.signRaw({ address, data: messageString ?? 'Default Sign Message', type: 'bytes' });
