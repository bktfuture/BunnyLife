...
function Component({ visible, className }: Props): React.ReactElement<Props> {
  ...
  const onSwitchNetwork = useCallback(
    async (slug: string) => {
      setLoading(true)
      const networkInfo = Object.values(NetworkInfo).find(
        network => network.slug === slug
      )
      if (networkInfo) {
        const network = chains.find(
          chain =>
            chain.label === networkInfo.name &&
            chain.namespace === networkInfo.namespace
        )
        if (network) {
          try {
            await set({
  ...
  return (
    <header
      className={CN('__wallet-header', className, {
        '-isMobile': !isWebUI
      })}
    >
      <div
        className={CN('wallet-header-content', {
          '-isConnected': visible,
        ...
        {visible ? (
          <>
            <div className={CN('__header-static')}>
              <div
                className={CN('__header-logo')}
                dangerouslySetInnerHTML={{ __html: LogoHeader }}
              />
              <div className={CN('__header-title')}>Bunny Life</div>
            </div>
          ...
          <>
            <div
              ...
              dangerouslySetInnerHTML={{ __html: LogoHeader }}
            />
            <div className={CN('__header-title', 'h5-text')}>SubConnect</div>
            <Button
